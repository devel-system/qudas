...
class AnnealFMQA(OptimizerMixin, IteratorMixin):
    """FMQAのアニーリング処理
    ...
    def __init__(self, blackbox, d: int, loop_num: int, token: str=None):
    ...
    def next_step(self, X, y=None, **iter_params) -> tuple:
        ...
        # self.result が重複しないようにする
        while (self.result == X).all(axis=1).any():
            flip_idx = rng.choice(np.arange(d))
            self.result[flip_idx] = 1 - self.result[flip_idx]

        # 推定された入力ベクトルを用いてブラックボックス関数を評価
        y_hat = self.blackbox(self.result)

        # 評価した値をデータセットに追加
        x = np.vstack((X, self.result))
        y = np.append(y, y_hat)
...
if __name__ == '__main__':
    ...
    steps = [
        ('TorchFMQA', TorchFMQA()),
        ('AnnealFMQA', AnnealFMQA(blackbox, d, loop_num=N, token="AE/p2lAwBrQpyGlHPKuMgpwbfQiO0OXXg6B"))
    ]
