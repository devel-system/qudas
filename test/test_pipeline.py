...
class TestPipeline(unittest.TestCase):
    ...
    def test_fit(self):
        ...
        params = {'alpha': 0.1}
        steps = [
            ('SimpleEstimatorStep', SimpleEstimatorStep(params)),  # Estimatorを使ったステップ
        ]
        pipeline = Pipeline(steps)
        pipeline.fit(X)

        self.assertEqual(pipeline.results['SimpleEstimatorStep'], None)  # サンプルでは predict しないためNone
    ...
    def test_optimize(self):
        ...
        steps = [
            ('SimpleOptimizerStep1', SimpleOptimizerStep()),  # OptimizerMixinを使ったステップ
        ...
        self.assertEqual(result['SimpleOptimizerStep2'], 24)  # {(X + 1) + 1} * 2 の結果を確認
    ...
    def test_iterator(self):
        """イテレータステップのテスト"""
        steps = [
            ('SimpleOptimizerStep', SimpleOptimizerStep()),  # OptimizerMixinを使ったステップ
        ]
        pipeline = Pipeline(steps, iterator=SimpleIteratorStep(loop_num=2))

        X = 10
        result = pipeline.optimize(X)
        self.assertEqual(result['SimpleOptimizerStep'], 26)  # {(X + 1 + 1) + 1} * 2 の結果を確認
