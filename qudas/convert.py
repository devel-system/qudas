...
### pulp
##############################
def pulp_to_dict(prob=pulp.LpProblem) -> dict:
    qubo = {}
...
def pulp_to_amplify(prob=pulp.LpProblem) -> Poly:
...
def pulp_to_pyqubo(prob=pulp.LpProblem) -> dict:
    qubo = 0
    for var in prob.objective.to_dict():
...
def pulp_to_matrix(prob=pulp.LpProblem) -> dict:
...
### amplify
##############################
def amplify_to_dict(prob=Poly) -> dict:
...
def amplify_to_pulp(prob=Poly) -> pulp.LpProblem:
...
def amplify_to_pyqubo(prob=Poly) -> dict:
...
def amplify_to_matrix(prob=Poly) -> np.ndarray:
...
### pyqubo
##############################
def pyqubo_to_dict(prob=Add) -> dict:

    model = prob.compile().to_qubo()

    return model[0]

def pyqubo_to_pulp(prob=Add) -> pulp.LpProblem:
...
def pyqubo_to_amplify(prob=Add) -> dict:
...
def pyqubo_to_matrix(prob=Add) -> np.ndarray:
...
### dict
##############################
def dict_to_pyqubo(prob=dict) -> dict:
...
def dict_to_pulp(prob=dict) -> pulp.LpProblem:
...
def dict_to_amplify(prob=dict) -> dict:
...
def dict_to_matrix(prob=dict) -> np.ndarray:
